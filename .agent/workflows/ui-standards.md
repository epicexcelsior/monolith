...
// turbo
```bash
grep -rn '#[0-9a-fA-F]\{6\}' apps/mobile/app/ apps/mobile/components/ --include='*.tsx' --include='*.ts' | grep -v 'node_modules' | grep -v 'theme.ts' | grep -v '.d.ts'
```
...
// turbo
```bash
grep -rn 'Dimensions.get' apps/mobile/app/ apps/mobile/components/ --include='*.tsx' --include='*.ts'
```
...
// turbo
```bash
grep -rn 'TouchableOpacity' apps/mobile/app/ --include='*.tsx' | grep -v 'node_modules'
```
...
// turbo
```bash
grep -rn "fontFamily:" apps/mobile/app/ --include='*.tsx' | grep -v 'FONT_FAMILY\.' | grep -v 'node_modules'
```
