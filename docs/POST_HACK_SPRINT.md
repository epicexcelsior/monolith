# Post-Hack Sprint — "Make It Click"

> **Branch:** `feat/post-hack-sprint` (create from `main`)
...
1. **Phase 1: Make It Make Sense** — Bug fixes + clarity + inspector redesign
2. **Phase 2: Make It Exciting** — Loot drops on charge (gacha dopamine)
3. **Phase 3: Polish + Ship** — End-to-end flow, visual tuning, APK build
...
3. [Phase 1: Make It Make Sense](#3-phase-1-make-it-make-sense)
4. [Phase 2: Make It Exciting](#4-phase-2-make-it-exciting)
5. [Phase 3: Polish + Ship](#5-phase-3-polish--ship)
...
|----------|-------------|
| Loot table + roll function | Define 12 items, rarity tiers, weighted roll logic |
| Zustand loot store | `inventory`, `pendingReveal`, `rollAndStore()`, AsyncStorage persistence |
| Roll on charge | In useBlockActions handleCharge, after charge succeeds call `rollAndStore(streak)` |
| Drop reveal UI | LootReveal.tsx: full-screen overlay, rarity flash, item card, equip button |
| Auto-equip | "Equip" button applies loot via existing customize handlers |
| Mount + timing | Add LootReveal to index.tsx. Queue after evolution celebration if both fire. |
...
Something will break. Unscheduled fix time.
...
export const LOOT_TABLE: LootItem[] = [
  ...
  { id: 'emoji_crown', name: 'Royal Crown', rarity: 'rare', type: 'emoji', value: '👑', description: 'A crown fit for a keeper' },
  { id: 'emoji_diamond', name: 'Diamond', rarity: 'rare', type: 'emoji', value: '💎', description: 'Brilliant and rare' },
  { id: 'emoji_lightning', name: 'Lightning', rarity: 'rare', type: 'emoji', value: '⚡', description: 'Pure energy' },
...
### Commits
- Format: `feat(post-hack): phase X.Y — [description]`
...
>>>>>>> worktree-feat/post-hack-sprint
