...
- **UI system** (liquid glass solarpunk palette, FloatingNav pills, TopHUD, bottom sheets, swipe-to-dismiss, tower reveal animation)
- **EAS Update / OTA** (expo-updates, channel: preview)
- **Server** (ownership enforcement, REST endpoints, hardened error handling)
- **Tapestry social layer** (on-chain profiles, follow/like/comment, deterministic block content IDs, namespace isolation — client-side, fire-and-forget)
- **Solana Blinks** (shareable poke URLs via memo transactions — dial.to cards, in-game poke effects)
- **MagicBlock SOAR** (on-chain leaderboard + 7 achievements, fire-and-forget, feature-flagged, active on devnet)
